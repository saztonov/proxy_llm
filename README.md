...
Управление — админ-сайт `/admin`: сайты и токены, справочник отделов и сотрудников, провайдеры, агентские ключи, статистика, аудит. Первый вход — встроенный `admin@test.com` / `qwertyui12345` (создаётся только на пустой базе); пароль сменить сразу, см. `docs/operator-guide.md` → «Первый вход».
