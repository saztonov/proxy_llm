...
### 4.4. Проверка idempotency

Две проверки: механизм прокси и реальное поведение клиента. Вторая — обязательная, и её нельзя пропускать: **клиент своего трафика в журнале не видит, поэтому недоработанную интеграцию может обнаружить только оператор.** На PassDesk это уже сработало вхолостую — 58 из 59 боевых сканов ушли без ключа, потому что заголовок добавили лишь в одно из двух мест вызова.

**а) Механизм прокси.** Имитировать retry — выполнить два curl с одинаковым `X-Idempotency-Key`:
...
**б) Реальный трафик клиента — обязательно, через сутки после запуска.** Проверить, что клиент действительно шлёт ключ из **всех** мест вызова, а не только из основного:

```bash
sqlite3 -header -column /var/lib/proxy_llm/prod.db \
  "select client_id,
          case when idempotency_key is null or idempotency_key='' then 'БЕЗ ключа' else 'с ключом' end k,
          count(*), round(avg(request_bytes)/1024.0) avg_kb
   from requests group by 1,2 order by 1;"
```

Строка «БЕЗ ключа» у клиента, у которого есть ретраи (BullMQ, Celery, cron), означает: дедуп у него не работает и каждый ретрай оплачивается как новый вызов. Большой `avg_kb` в этой строке — признак, что мимо дедупа идёт как раз основной трафик. Сообщить владельцу сервиса, ссылка для него — §5.1 скилла `connect-proxy-llm`. Повторять эту проверку при каждом онбординге нового клиента.
...
конкурентности, при желании — своя модель, свой белый список моделей и свой ключ OpenRouter.
Остаётся синхронным шлюзом (никакого брокера/воркера на прокси — у клиентов свои очереди).
...
Политика моделей задаётся полем `allowedModels`:

| Значение | Смысл |
|---|---|
| `[]` (или поле не задано при пустом `CLIENT_DEFAULT_ALLOWED_MODELS`) | клиент модель **не выбирает** — присланный `model` игнорируется, форс `defaultModel` + `fallbackModels` |
| список | клиент может прислать `model` из него; иначе → `400 model_not_allowed` (в теле — `allowed`) |
| `["*"]` | любая модель OpenRouter |

Поле **не задано** → наследуется `CLIENT_DEFAULT_ALLOWED_MODELS` из `.env`; явный `[]`
этот дефолт **перебивает** (единственный способ удержать клиента на дефолт-модели, когда
выбор включён глобально). Заглушки `proxy`/`default`/`auto` в поле `model` всегда означают
«модель не выбираю» → `defaultModel` + `fallbackModels`. **Явный выбор модели отключает
fallback-цепочку** — в OpenRouter уходит одиночный `model` без `models[]`.

⚠️ **Перед включением выбора существующему клиенту — чек-лист `docs/vps-update.md` §4a.**
Клиенты уже шлют поле `model`, просто оно игнорируется: подключённые по скиллу шлют заглушку
...
4. Передать клиенту токен по защищённому каналу + его политику моделей: `clientId`,
   `defaultModel` и `allowedModels` (пусто / список / `*`) — от этого зависит, может ли он
   слать `model`. Инструкция подключения — скилл `.claude/skills/connect-proxy-llm/`
   (там же §1 — три варианта политики глазами клиента, и §9 «Переход с варианта A на
   вариант B» — рецепт доработки уже подключённого проекта).

   Скилл клиентский целиком: операторских путей и команд в нём нет, поэтому его можно
   отдавать порталу как есть, без вычитки. Не переносите в него содержимое этого раздела.
...
### 8.4. Учёт расходов: сколько потратил каждый потребитель

**Где смотреть:** `/dashboard/billing` (тот же Basic Auth, что у `/dashboard`), фильтры
`?from=YYYY-MM-DD&to=YYYY-MM-DD`, по умолчанию последние 30 суток. То же в JSON —
`/dashboard/billing.json`. Плюс денежная секция в ежедневном дайджесте Telegram.

**Откуда берутся деньги.** OpenRouter возвращает фактически списанную сумму прямо в теле
каждого ответа (`usage.cost`), и прокси пишет её по каждой попытке. Ничего включать не надо,
дополнительных запросов к OpenRouter это не стоит.

**Две таблицы, и это важно:**

- `requests` — входящие HTTP-запросы клиентов (нагрузка, latency, ошибки);
- `billing_attempts` — фактические обращения к OpenRouter, **одна строка = один платный
  вызов**. Все денежные и токенные отчёты считаются по ней.

Считать деньги по `requests` нельзя. Два клиентских запроса с одинаковым `X-Idempotency-Key`
разделяют один upstream-вызов и одно списание — в `requests` они дают две строки (вторая
помечена `dedup_join=1`), а в `billing_attempts` одну. И наоборот: один запрос с ретраем даёт
одну строку в `requests`, но две оплаченные попытки в `billing_attempts`, каждую со своими
токенами и своей моделью.

**Три состояния стоимости.** На странице они разнесены намеренно:

| состояние | что значит | можно ли нести в бухгалтерию |
|---|---|---|
| факт | `usage.cost` от OpenRouter | да |
| ≈ оценка | расчёт по каталожной цене, факт не пришёл | **нет** |
| missing | нет ни факта, ни пригодной оценки | нет, но и игнорировать нельзя |

Оценка **никогда** не суммируется с фактом: каталог публикует минимальную цену модели, а не
цену провайдера, на которого фактически ушёл роутинг. Там, где посчитать честно нельзя
(роутеры с ценой `-1`, тарифные ступени по длине промпта, `:online`/web search), оценки нет
вовсе — пустая клетка честнее правдоподобного, но неверного числа.

`missing` появляется, когда тело ответа не удалось разобрать: клиент оборвал соединение,
сработал таймаут попытки, ответ превысил `UPSTREAM_RESPONSE_BODY_LIMIT_BYTES`. Генерация при
этом могла быть оплачена. Растущий счётчик `missing` — повод посмотреть, не рвутся ли запросы
по дедлайну.

**История цен.** Раз в сутки (по умолчанию 06:00 МСК) прокси забирает каталог
`GET /api/v1/models` и пишет новую версию цены только для тех моделей, у которых `pricing`
изменился; факт проверки фиксируется в `price_sync_runs`. Оценка всегда берёт цену,
наблюдавшуюся **на момент запроса**, а не сегодняшнюю. История не удаляется: каталог отдаёт
только текущие цены, задним числом её не восстановить.

Если синхронизация отстала, дайджест напишет об этом. На факт (`usage.cost`) это не влияет —
страдает только оценка.

**Расхождение с инвойсом OpenRouter гарантировано** — полный список причин и команды проверки
в `docs/vps-update.md`, раздел 7. Коротко: наши сутки по Москве, инвойс по UTC; оплаченные
попытки, отброшенные ретраем; `missing`; расход мимо прокси; разные payer scope.

**Лимитов нет.** Учёт только показывает расход; при перерасходе запросы не блокируются.

---

## Админ-сайт и агентский контур

С версии 2026-09 у прокси два контура и админ-сайт:

- **Сайты** (`/api/v1`) — прежний контур порталов, поведение не менялось; реестр клиентов и токенов теперь в SQLite и правится в `/admin` без рестарта.
- **Агенты** (`/agent/v1`) — OpenAI API для Cursor и других агентов сотрудников; подробно — `docs/agents.md`.
- **Админка** (`/admin`) — вход только для администраторов; сайты и токены, справочник отделов и сотрудников, провайдеры, агентские ключи, статистика, журнал запросов, аудит действий.

Разовый переход с прежней версии — `docs/vps-update.md`, §0a.

### Импорт справочника из Excel

«Справочник» → «Импорт из Excel»: файл `.xlsx` до 1 МБ, первый лист, столбцы «ФИО» и «Отдел» (заголовок ищется в первых 10 строках, прочие столбцы не учитываются).

- Отдел, которого нет, создаётся; сравнение имён — без учёта регистра, лишних пробелов и разницы «ё/е». Slug получается транслитерацией (`Отдел закупок` → `otdel-zakupok`).
- Новый сотрудник добавляется с логином из ФИО (`Иванов Иван Иванович` → `ivanov.ii`, при совпадении `ivanov.ii2`). Логин — технический идентификатор для журнала, а не учётная запись: входа у сотрудников нет, работают они по агентскому ключу.
- Сотрудник, чьё ФИО уже есть в справочнике (в любом отделе), и повтор ФИО внутри файла пропускаются. Импорт того же файла второй раз ничего не создаёт.
- Итог показывается сводкой: сколько добавлено и пропущено, какие отделы созданы, какие строки с ошибками. В аудит пишется одна запись `directory.import` только со счётчиками.

Файл разбирается на сервере в отдельном процессе с лимитами времени и памяти; старый формат `.xls` не принимается — пересохраните как `.xlsx`.

### Первый вход

На базе без администраторов сервис при старте создаёт встроенного **`admin@test.com` / `qwertyui12345`**. Эта пара опубликована в репозитории: смените пароль сразу после входа («Настройки» → «Смена пароля»; интерфейс сам туда ведёт и показывает баннер, пока пароль по умолчанию не сменён, а сервис пишет предупреждение в лог на каждом старте). Если админ уже есть, встроенный не создаётся и ничего не сбрасывается. Завели свой логин — выключите встроенного: `admin.js disable --login admin@test.com`.

### Как устроена авторизация админки

- Пароль — scrypt; неверный пароль, неизвестный и отключённый логин дают одинаковый ответ за одинаковое время. После `ADMIN_LOGIN_MAX_ATTEMPTS` неудач логин блокируется на окно, отдельно действует лимит попыток с IP; серия неудач — алерт в Telegram.
- Access-токен (JWT, 15 минут) и refresh-токен лежат в cookie `httpOnly`, `SameSite=Strict`, `Secure`: JavaScript страницы их не видит. Refresh уходит только на `/admin/api/auth`.
- Каждый refresh выдаёт новую пару; повторное предъявление старого refresh — признак кражи: вся сессия отзывается, в Telegram уходит алерт.
- Каждый запрос сверяет сессию с БД, поэтому выход, смена пароля и `revoke-sessions` действуют сразу и переживают рестарт.
- CSRF: токен, привязанный к сессии, в заголовке + обязательная проверка Origin для всех изменяющих запросов; только JSON. CSP запрещает inline-скрипты и встраивание страниц.
- Все изменения пишутся в аудит (кто, когда, с какого IP, что); ключи и токены туда не попадают.

### Секреты и восстановление

| Ситуация | Действие |
|---|---|
| забыт пароль админа | `admin.js reset-password --login admin --generate` — отзывает все его сессии |
| разлогинить всех | `admin.js revoke-sessions` |
| выключить админа (например, встроенного) | `admin.js disable --login admin@test.com` — последнего включённого выключить нельзя |
| ротация `ADMIN_JWT_SECRET` | заменить в `.env`, рестарт; текущие access перестанут проверяться, вход — заново через refresh или пароль |
| ротация `SECRETS_ENCRYPTION_KEY` | сначала `printf '%s' "$NEW_KEY" \| admin.js rekey` (старый ключ — из окружения), затем заменить в `.env` и рестарт |
| утерян `SECRETS_ENCRYPTION_KEY` | сервис не стартует, пока в БД есть значения под старым ключом. Обнулить `providers.api_key_enc`, `providers.extra_headers_enc`, `site_clients.openrouter_api_key_enc` (и их `*_fp`), стартовать с новым ключом, ввести ключи провайдеров и собственные ключи сайтов в админке заново |

Запуск CLI на VPS — от пользователя сервиса и с его окружением:

```bash
cd /opt/proxy_llm
sudo -u proxy_llm bash -c 'set -a; . /etc/proxy_llm/.env; set +a; /opt/node-v22/bin/node dist/cli/admin.js <команда>'
```
