...
Строка «БЕЗ ключа» у клиента, у которого есть ретраи (BullMQ, Celery, cron), означает: дедуп у него не работает и каждый ретрай оплачивается как новый вызов. Большой `avg_kb` в этой строке — признак, что мимо дедупа идёт как раз основной трафик. Сообщить владельцу сервиса, ссылка для него — §4.1 скилла `connect-proxy-llm`. Повторять эту проверку при каждом онбординге нового клиента.
...
4. Передать клиенту токен по защищённому каналу + его политику моделей: `clientId`,
   ...
   (там же §8 «Как начать выбирать модель» — рецепт доработки уже подключённого проекта).
