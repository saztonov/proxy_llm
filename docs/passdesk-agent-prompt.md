...
1. В `server/src/services/ocr/ocrService.js`:
   а) Найти ВСЕ места, где собираются HTTP-headers для axios.post в OpenRouter.
      ...
      **Не полагаться на эти номера строк — найти грепом и обработать каждое:**
      `grep -rn "config.endpoint\|OCR_OPENROUTER_ENDPOINT\|chat/completions" server/src/ --include='*.js'`

      ⚠️ Эту задачу уже делали и сделали НАПОЛОВИНУ: заголовки добавили в одно
      место, а второе — то, по которому идёт основной боевой OCR, — пропустили.
      По журналу прокси за 2026-07-16: из 59 запросов PassDesk 58 ушли БЕЗ
      `X-Idempotency-Key` (средний размер тела 505 КБ — настоящие сканы). Дедуп
      не сработал ни разу, каждый ретрай BullMQ оплачивался как новый вызов
      OpenRouter. Правка считается сделанной, только когда покрыты ВСЕ места.
...
6. **Каждое** найденное в п.1а место вызова покрыто заголовками. Отчитаться
   списком: файл + строка + вывод грепа, — а не «сделано».
7. Прогнать OCR по всем типам документов/сценариям, какие есть в UI, чтобы
   задействовать все места вызова, а не только основное.
8. **Финальная проверка на стороне оператора прокси** (сам PassDesk своего
   трафика в журнале не видит, поэтому иначе пропуск не поймать):
   `select case when idempotency_key is null or idempotency_key='' then 'БЕЗ' else 'с' end,
    count(*) from requests where client_id='passdesk' group by 1;`
   Строки «БЕЗ» быть не должно. Пока она есть — задача не выполнена.
