...
1. В `server/src/services/ocr/ocrService.js`:
   ...
   г) Не менять структуру payload (`messages`, `model`, и т.п.). **Важно:** прокси
      сам перезапишет `model` (или установит `models`) на свою и УДАЛИТ из
      payload следующие поля, если они там есть: `provider`, `route`,
...
2. В README или в `docs/ocr.md` PassDesk:
   ...
   - Указать env-переменные, которые нужно настроить:
     ...
     * OCR_API_KEY=<PROXY_INBOUND_TOKEN> (НЕ openrouter-ключ)
     * OCR_IDEMPOTENCY_VERSION=v1   # поднимать до v2 при изменении промпта/схемы распознавания
   - Указать, что значения OCR_OPENROUTER_MODEL и OCR_FALLBACK_MODEL формально
     остаются, но фактически игнорируются прокси (он управляет моделью сам).
...
- НЕ удалять и не переименовывать OCR_OPENROUTER_ENDPOINT / OCR_API_KEY /
  ...
  совместимости с существующей логикой `getOcrConfig()`.
