...
**Где ведутся токены:** с версии с админкой — в БД, только как sha256: выпуск и отзыв в `/admin` → «Сайты», открытый токен показывается один раз. До неё — `/etc/proxy_llm/clients.json` на VPS. В git реального реестра нет и быть не должно — только шаблон `deploy/clients.example.json`. Второе место хранения открытого токена — менеджер секретов (1Password/Vaultwarden): после выдачи взять его больше неоткуда.

**Интерфейс:** с версии с админкой онбординг делается в `/admin` → «Сайты»: клиент, политика моделей, лимиты, токен. Шаги ниже — ручной путь для версии до неё; `/dashboard` по-прежнему только мониторинг.
...
```bash
systemctl restart proxy_llm      # версия до админки: реестр читается только на старте, ExecReload нет
...
- подключённые по скиллу (`fot`, `matcheck`, `estimat`) шлют заглушку `"proxy"` — **безопасны**,
  заглушка и после включения означает «модель не выбираю». `fot` по этой причине уже переведён
  в `["*"]` (20.07.2026) — заглушка у него продолжает работать, проверено;
...
- **Денежных лимитов и блокировок тоже нет.** Учёт расходов (§7) только показывает, кто сколько потратил; при перерасходе запросы не останавливаются.
...
Пер-клиентская статистика по нагрузке:
...
Расход в деньгах считается **не по этой таблице**, а по `billing_attempts` — см. §7.

---

## 7. Учёт расходов (биллинг)

Появился в миграции `003`. Отдельного шага применения **не требуется** — миграции идемпотентно накатываются при старте сервиса, `npm run migrate` по-прежнему запускать нельзя.

### Что и где считается

Учёт двухуровневый, и это принципиально:

| таблица | что в ней | на какой вопрос отвечает |
|---|---|---|
| `requests` | входящие HTTP-запросы клиентов | сколько запросов пришло, latency, ошибки |
| `billing_attempts` | фактические обращения к OpenRouter, **одна строка = один платный вызов** | сколько денег и токенов |

Считать деньги по `requests` **нельзя**: там есть dedup-join'ы (несколько HTTP-запросов клиента с одним `X-Idempotency-Key` разделяют один upstream-вызов и одно списание), а токены в них — только от последней попытки. Все денежные отчёты идут по `billing_attempts`.

### Три состояния стоимости

- **факт** (`usage_source='response'`, `cost_usd`) — сумма, реально списанная OpenRouter. Только её можно нести в бухгалтерию.
- **≈ оценка** (`cost_est_usd`) — расчёт по каталожной цене, когда факт не пришёл. **Никогда не суммируется с фактом**: каталог публикует минимальную цену модели, а не цену провайдера, на которого ушёл роутинг.
- **missing** — нет ни того, ни другого. Возникает, когда тело ответа не разобрано: обрыв клиента, таймаут попытки, ответ больше `UPSTREAM_RESPONSE_BODY_LIMIT_BYTES`. Генерация при этом могла быть оплачена. Это дыра покрытия по построению, поэтому она показывается отдельным счётчиком, а не прячется.

`est_quality` объясняет, почему оценки нет: `unpriceable` — считать нечем в принципе (роутер с ценой `-1`, тарифные ступени по длине промпта, `:online`/web search), `no_price` — цена модели на тот момент не наблюдалась, `partial` — цена кэша не опубликована, использована обычная цена промпта (оценка завышена на скидку за кэш).

### Где смотреть

```bash
# Страница расходов (Basic Auth, тот же, что у /dashboard)
https://<домен>/dashboard/billing
https://<домен>/dashboard/billing?from=2026-07-01&to=2026-07-20

# JSON тем же набором фильтров
curl -sS -u admin:<DASHBOARD_PASS> \
  'http://127.0.0.1:3000/dashboard/billing.json?from=2026-07-01&to=2026-07-20' | python3 -m json.tool

# Additive-блок billing в существующем stats.json
curl -sS -u admin:<DASHBOARD_PASS> http://127.0.0.1:3000/dashboard/stats.json | python3 -m json.tool
```

Плюс денежная секция в ежедневном Telegram-дайджесте (09:00): вчера, текущий день с пометкой «неполные сутки», 30 суток, разбивка по клиентам.

### Проверка после обновления

```bash
# 1. Миграция применилась
sqlite3 /var/lib/proxy_llm/prod.db ".tables" | tr ' ' '\n' | grep billing

# 2. Факт стоимости пишется (после первого же боевого запроса)
sqlite3 /var/lib/proxy_llm/prod.db \
  "select billing_day, client_id, model_used, prompt_tokens, completion_tokens,
          cost_usd, usage_source
   from billing_attempts order by id desc limit 10;"

# 3. Расход по клиентам за сутки
sqlite3 /var/lib/proxy_llm/prod.db \
  "select billing_day, client_id, count(*) attempts,
          sum(prompt_tokens) input, sum(completion_tokens) output,
          round(sum(case when usage_source='response' then cost_usd else 0 end), 6) actual_usd
   from billing_attempts group by billing_day, client_id order by billing_day desc limit 20;"

# 4. Синхронизация цен
sqlite3 /var/lib/proxy_llm/prod.db \
  "select run_day, ok, models_seen, versions_written, error from price_sync_runs order by id desc limit 5;"
journalctl -u proxy_llm | grep price_sync
```

**Первая синхронизация цен появится не раньше `BILLING_PRICE_SYNC_HOUR` (по умолчанию 06:00 МСК).** Если сервис перезапущен позже этого часа и снимка за сегодня ещё нет, он догонит его сам в течение минуты после старта. При неудаче — повтор раз в час до конца суток. Отсутствие снимка на факт (`usage.cost`) не влияет, страдает только каталожная оценка.

### Почему цифры не сойдутся с инвойсом OpenRouter

Это ожидаемо. Причины, в порядке величины:

1. **Сутки считаются по `BILLING_TIMEZONE` (Europe/Moscow), OpenRouter отчитывается по UTC.** Суточные суммы не совпадут никогда; месячные разойдутся на трёхчасовой хвост на границе месяца.
2. **Попытки, отброшенные ретраем.** Генерация оплачена, результат клиенту не отдан. Сумма видна на `/dashboard/billing` отдельной карточкой «Потери на ретраях».
3. **`missing`-попытки** — оплачены, но данных нет (см. выше).
4. **Расход мимо прокси** — прямые вызовы OpenRouter тем же ключом в журнале не видны.
5. **Разные payer scope.** Если клиенту выдан свой `openrouterApiKey`, его расход уходит на другой аккаунт. В `billing_attempts` это видно по `payer_scope` и `api_key_fp` (16 hex от sha256 ключа — различает аккаунты и переживает ротацию).
6. **Учёт начат не с нуля.** Дата в `billing_meta['accounting_started_at']` — момент первой миграции. Для запросов до неё стоимости нет и восстановить её нельзя: пересчёт по сегодняшним ценам дал бы правдоподобные, но неверные числа. Пустой период до этой даты — не «ничего не тратили».

### Рост базы

Порядок +5-6 МБ в год при текущем трафике (десятки запросов в сутки): `billing_attempts` ~350 Б на попытку, история цен — ~400 строк на старте и десятки в год дальше (новая версия пишется только при изменении цены). Ретеншна у истории цен нет намеренно — каталог отдаёт только текущие цены, задним числом историю не восстановить. Отдельных действий по бэкапу не нужно: всё лежит в `prod.db`, который целиком копирует `scripts/backup-db.sh`.
