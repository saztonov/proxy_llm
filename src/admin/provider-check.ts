...
function scrub(text: string, key: string | null): string {
  const t = key ? text.split(key).join('[REDACTED]') : text;
  return sanitizeForLog(t, 300);
}
 ...
 * Ответ провайдера в ошибке обрезается и чистится от ключа.
 */
export async function checkProvider(p: AgentProvider, timeoutMs: number, dispatcher?: Dispatcher): Promise<ProviderCheckResult> {
  ...
  try {
    ...
    if (models.status !== 200) return fail(models.status, scrub(models.text, p.apiKey));
    ...
    return fail(null, scrub(sanitizeErrorForLog(err).message, p.apiKey));
  }
}
