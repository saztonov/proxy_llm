...
## 4. Заголовки трассировки и дедупа
...
### 4.1 `X-Idempotency-Key` — обязателен при ретраях
...
```bash
# подставьте свой способ вызова
grep -rn "chat/completions\|PROXY_LLM_BASE_URL\|OCR_OPENROUTER_ENDPOINT" src/ server/ --include='*.js' --include='*.ts' --include='*.py'
```
...
во всех местах вызова (§4.1)**. Он правится там же, а стоит дороже: пропущенное место вызова
...
1. **Проверить, что вы шлёте сейчас.** Найдите в коде **все** места вызова (grep из §4.1) и
   посмотрите, что каждое из них кладёт в поле `model`. Три случая:
...
4. Передать токен по защищённому каналу и **сообщить политику моделей**: `clientId`,
   ...
   клиент слать `model`.
5. **Через сутки после запуска — проверить дедуп клиента.** Клиент свой трафик в журнале не
   видит, поэтому недоработанное место вызова обнаружить может только оператор:
   ...
   Ненулевая строка «БЕЗ ключа» у клиента, у которого есть ретраи, — это его ретраи,
   оплаченные дважды. Сообщить владельцу сервиса со ссылкой на §4.1.
...
- [ ] Найдены **все** места вызова прокси (grep из §4.1), а не только основной путь.
...
for i in 1 2; do
  curl -s -o /dev/null -w "%{http_code} " "$BASE/api/v1/chat/completions" \
    -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -H "X-Idempotency-Key: $KEY" -H "X-Request-Id: $(uuidgen)" \
...
Главная проверка — **боевая, после запуска**: попросите оператора показать долю ваших
запросов без ключа. Она обязана быть нулевой:

```sql
select case when idempotency_key is null or idempotency_key='' then 'БЕЗ ключа' else 'с ключом' end,
       count(*), round(avg(request_bytes)/1024.0) avg_kb
from requests where client_id='<ваш clientId>' group by 1;
```

Строка «БЕЗ ключа» с ненулевым счётчиком означает, что какое-то место вызова осталось
недоработанным — и именно оно, скорее всего, несёт основной трафик.
